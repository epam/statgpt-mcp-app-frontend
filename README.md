...
An **inner-iframe MCP app** for the `<private-repo>` (the inner widget renders an SDMX data explorer). The prototype's
outer chart widget can host a second, cross-origin `<iframe>` and act as its agent
bridge (see `<private-repo>/docs/007-nested-iframe/`). This repo is a real
...
`<private-repo>/scripts/set-inner-widget.ps1`.
