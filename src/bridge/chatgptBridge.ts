...
declare global {
  ...
  interface WindowEventMap {
    "openai:set_globals": CustomEvent<{
      globals?: { toolInput?: unknown; toolOutput?: unknown; widgetState?: unknown };
    }>;
  }
}
...
function readHostContext(): McpUiHostContext {
  return {
    displayMode: (window.openai?.displayMode as McpUiHostContext["displayMode"]) ?? "inline",
...
export function createChatGPTBridge(): HostBridge {
  ...
  function onSetGlobals(e: Event) {
    const globals = (e as CustomEvent<{ globals?: { toolInput?: unknown; toolOutput?: unknown } }>).detail?.globals ?? {};
    ...
    if (globals.toolOutput !== undefined) {
      patch({ toolResult: globals.toolOutput, hostContext: readHostContext() });
    } else {
