...
describe('InfoIcon', () => {
  ...
  it('renders an svg element for claude + desktop', () => {
    const { container } = render(<InfoIcon platform={Platform.Desktop} />);
    expect(container.querySelector('svg')).toBeInTheDocument();
  });

  it('renders an svg element for claude + mobile', () => {
    const { container } = render(<InfoIcon platform={Platform.Mobile} />);
    expect(container.querySelector('svg')).toBeInTheDocument();
  });
  ...
  it('forwards width/height props to the underlying icon', () => {
    const { container } = render(
      <InfoIcon platform={Platform.Desktop} width={18} height={18} />,
    );
